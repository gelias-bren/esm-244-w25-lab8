...
```{r setup}
library(spatstat) # list objects to handle spatial data best. 
library(terra) 
library(tidyverse)
library(here)
library(sf)
...
humboldt_sf <- read_sf(dsn = here("data/voledata"), 
  ...
  st_transform(crs = st_crs(voles_sf)) # Make sure CRS match to overlay correctly in same location
...
voles_full <- ppp(voles_ppp$x, voles_ppp$y, window = humboldt_win)
  # Translation of window leads to one point being outside of window
...
# Sigma is the assumed variance around these points. Balance between overfitting and underfitting. Subjective
...
# we see p  = 0.0198 and the G_obs is above the G_theoretical for Complete Spatial Randomness, so we can say we are clustered
```
...
colnames(gfunction_out)

gfunction_df <- as.data.frame(gfunction_out) |>
  pivot_longer(cols = -r, names_to = 'model')

ggplot(gfunction_df, aes(x = r, y=value, color = model))+
  geom_line()+
  theme_bw()
```
...
lfunction_out <- envelope(voles_full, fun = Lest, r = r_vec, 
                          nsim = 10) 
```
...
lfunction_out ### Check the output of gfunction, then...
plot(lfunction_out)

```

So the K function shows much more certainty that the data is clustered.
